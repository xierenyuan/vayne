...
module.exports = (config, utils) => {
  let loaders = []
  let webpackPlugins = []
  // 最前的插件 默认不处理会在开发环境 和生产环境中追加 以可以在插件中判断只在某个环境添加
  let beforePlugins = []
  // 最后的插件
  let afterPlugins = []
  ...
  function requirePlugin(name) {
    ...
    try {
      ...
      let plugin = new Plugin(config, log, utils)
      loaderConfig(plugin)
    } catch (error) {
      console.log(error)
  ...
  function loaderConfig(plugin) {
    ...
    // 放在最前的插件处理
    if (_.isArray(plugin.beforePlugins)) {
      beforePlugins = _.union(beforePlugins, plugin.beforePlugins)
    }
    // 放在最后的插件处理
    if (_.isArray(plugin.afterPlugins)) {
      afterPlugins = _.union(afterPlugins, plugin.afterPlugins)
    }
  }
  ...
  plugins.forEach(plugin => {
    ...
    if (_.isObject(plugin)) {
      loaderConfig(new plugin(config, log, utils)) // eslint-disable-line
      return
    }
