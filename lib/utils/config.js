...
module.exports = function(env, cwd) {
  ...
  const argv = JSON.parse(process.env[process.env.NODE_ENV === 'production' ? 'VAYNE_BUILD_ARGV' : 'VAYNE_SERVER_ARGV'] || {})
  // 如果指定了配置文件 使用 指定的配置文件
  if (argv.config) {
    return _.merge(defaultConfig, getConfigFile(argv.config, paths))
  }
  let config = getConfig('.vaynerc', paths)
  let newConfig = _.merge(defaultConfig, config)
  // add version
  let version = require(paths.appPackageJson).version || '1.0'
  newConfig.dev.env.VERSION = `"${version}"`
  newConfig.build.env.VERSION = `"${version}"`
  return newConfig
}
